...
router.delete('/delete/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // 3. Delete User (wrap in try for safety)
    try {
      ...
      return res.status(500).json({ message: 'Failed to delete user', error: userError.message });
    }

    // 4. Delete Progress
try {
  console.log("Attempting to delete Progress for playerId:", id);
  const progressResult = await Progress.deleteOne({ playerId: id });
  progressDeletedCount = progressResult.deletedCount || 0;
  console.log("Progress delete result:", progressResult);
} catch (progressError) {
  console.error('Progress delete error:', progressError.stack);
}
    ...
    return res.status(200).json({
      ...
      deletedUser,
      progressDeleted: progressDeletedCount > 0 ? 'Yes' : 'No (none found)'
    });

  } catch (error) {
    console.error('Overall Delete Route Error:', error.stack);
